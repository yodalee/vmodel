...
void varray_unpack(T &t, const vint<false, num_bit> u,
                   ::std::integer_sequence<unsigned, i...> idx);
template <typename T, unsigned num_bit, unsigned... i>
void vstruct_unpack(T &t, const vint<false, num_bit> u,
                    ::std::integer_sequence<unsigned, i...> idx);
// template<typename T, unsigned num_bit, unsigned ...i>
// void vunion_unpack(
...
void vstruct_unpack(T &t, const vint<false, num_bit> u,
                    ::std::integer_sequence<unsigned, i...> idx) {
  ((unpack_nocheck(t.template get<i>(),
                   u.template Slice<bit_offset_first_member_at_msb_v<T, i>,
...
void unpack_nocheck(T &t, const vint<false, num_bit> u) {
  ...
  if constexpr (is_vint_v<T>) {
    ...
    detail::varray_unpack<T>(
        t, u, ::std::make_integer_sequence<unsigned, T::asize>());
  } else if constexpr (is_vstruct_v<T>) {
    detail::vstruct_unpack<T>(
        t, u, ::std::make_integer_sequence<unsigned, T::num_members>());
...
void unpack(T &t, const vint<false, num_bit> &u) {
  static_assert(bits_v<T> == num_bit,
                "Unpack and vint must have the same bitwidth");

...
auto varray_pack(const T &src, ::std::integer_sequence<unsigned, i...> idx);
template <typename T, unsigned... i>
auto vstruct_pack(const T &src, ::std::integer_sequence<unsigned, i...> idx);
// template<typename T, unsigned ...i>
// auto vunion_pack(
// 	const T &src,
...
template <typename T> auto vint_pack(const T &src) {
...
template <typename T> vint<false, bits<T>()> pack(const T &t) {
  ...
  if constexpr (is_vint_v<T>) {
    ...
    return detail::varray_pack<T>(
        t, ::std::make_integer_sequence<unsigned, T::asize>());
  } else if constexpr (is_vstruct_v<T>) {
    return detail::vstruct_pack<T>(
        t, ::std::make_integer_sequence<unsigned, T::num_members>());
